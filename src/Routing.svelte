<script>
  ...
  onMount(async () => {
    try {
      //const res = await fetch(`/uploads/${params.first}.pdf`);
...
.then(data => {
  ...
  if(message!="wrong token"){state=true;  }
  imgBinary=message.data;
  // ... do something with the message and emailToken ...
});
  ...
  async function addPDF(file) {
    ...
    //const imgBlob = await (await fetch(`/storedImages/${params.first}.png`)).blob();
    // Assuming you have a buffer object named "bufferData"
    //console.log(imgBinary);
    //console.log(imgBlob);

    const uint8Array = new Uint8Array(imgBinary.data);
...
addImage(blob);
  ...
  async function savePDF() {
    ...
    let editedPDFData;
    try {
      editedPDFData = await save(pdfFile, allObjects, params.first, pagesScale);
    } catch (e) {
      console.log(e);
    } finally {
      saving = false;
  ...
  <label
    ...
    <div
      ...
      <label
        ...
        <img src="signingpen.png" alt="An icon for adding text" />
      </label>
    </div>
    <div class="justify-center mr-3 md:mr-4 w-full max-w-xs hidden md:flex">      <img src="/encryption.png"  height="40" width="40" class="mr-2" alt="a pen, edit pdf name" />
